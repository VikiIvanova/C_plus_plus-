...
{
    int notClosedOpenSmallParenthesis = 0;
    int notClosedOpenMediumParenthesis = 0;
    int notClosedOpenLargeParenthesis = 0;
    for(char c : expr)
    {
        if(isOpeningSmallParenthesis(c))
        {
...
{
    return isClosingSmallParenthesis(c) || isClosingLargeParenthesis(c) || isClosingMediumParenthesis(c);
}

bool isMathingParenthesis(char opening, char closing)
{
    ...
    if(isClosingLargeParenthesis(opening))
    {
...
bool hasValidParenthesis(const std::string& expr)
{
    ...
    for(char c : expr)
    {
        ...
        {
            if(!isMathingParenthesis(notClosedOpenParenthesis.pop(), c))
            {
                return false;
            }
        }
