...
int peopleAwareOfSecretv2(int days, int delay, int forget)
{
    ...
    for(int day = 1; day < days; day++)
    {
        ...
        SecretSpread newSecretSread = {delay, forget, 0};
        while(!ppl.empty())
        {
            ...
            {
                newSecretSread.numberOfPeople += p.numberOfPeople;
            }
        }
        if(newSecretSread.numberOfPeople > 0)
        {
            nextDayPpl.enqueue(newSecretSread);
        }
        ppl = nextDayPpl;
    }
