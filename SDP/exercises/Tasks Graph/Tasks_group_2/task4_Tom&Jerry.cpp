/*Том гони Джери из правоъгълна стая разделена на
квадрати. Всеки от квадратите може да бъде празен или да е зает
от нещо. Тъй като и двамата се движат с една и съща скорост,
единствената цел на Джери е да стигне от горния ляв ъгъл на
стаята до долния десен, където се намиранеговият дом, възможно
най-бързо. Помогнете на Джери, като му кажете по кой път да
мине.*/
...
int main()
{
    ...
    if(map[HEIGHT - 1][WIDTH - 1] != Status::FREE && map[HEIGHT - 1][WIDTH - 1] != Status::TAKEN)
    {
        ...
        while(!(curr.col == 0 && curr.row == 0))
        {
            ...
            //add move to the previous coord
