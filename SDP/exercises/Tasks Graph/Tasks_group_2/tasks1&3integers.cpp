/*Задача 1. Намерете броя на свързаните компоненти в
неориентиран граф.
Задача 3. Проверете дали в даден ориентиран граф има цикъл*/
...
bool hasEdge( const GraphAdjLists& graph, size_t from, size_t to)
{
...
void addEdgeOriented(GraphAdjLists& graph, size_t from, size_t to)
{
    if(!hasEdge(graph, from, to))
       graph[from].push_front(to);
}

void addEdgeUnoriented(GraphAdjLists& graph, size_t from, size_t to)
{
...
//Task1 - Find the number of connected components in an undirected graph
