...
bool hasEdge(const GraphAdjLists &graph, const std::string &from, const std::string &to)
{
...
void addEdgeOriented(GraphAdjLists &graph, const std::string &from, const std::string &to)
{
    if (!hasEdge(graph, from, to))
        graph[from].push_back(to);
}

void addEdgeUnoriented(GraphAdjLists &graph, const std::string &from, const std::string &to)
{
...
size_t countComponents(const GraphAdjLists &graph)
{
    ...
    {
        // pair->first is the key(each vertex)
...
// Task 3 - Check is an oriented graph has a cycle
bool hasCycleOriented(const GraphAdjLists &graph)
{
    ...
    // graph.begin() is an iterator to the fisrt element in the map
    //->fisrt is its value(our vertex identifier)
