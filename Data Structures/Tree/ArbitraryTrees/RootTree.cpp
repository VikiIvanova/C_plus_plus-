...
inline RootTree<ValueType, Container>::Node(Node* parent, const ValueType& value)
      : fParentPtr(parent), fValue(value) {}
...
template<class ValueType, class Container>::RootTree():fRootPtr(nullptr) {}
...
RootTree<ValueType, Container>::deleteNode(Node* node)
{
    if(Node* parent = node->fParentPtr)
    {
        ...
        typename ChildrenList::const_iterator itNode = std::find(parentChildren.begin(), parentChildren.end(), node);
