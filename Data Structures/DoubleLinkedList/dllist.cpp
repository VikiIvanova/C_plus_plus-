...
template <class T>
DLList<T>::DLList() : fisrt(nullptr), last(nullptr){}
...
DLList<T>& DLList<T>::pushInFront(const T& data)
{
    if(first != nullptr && last != nullptr)
    {
        fisrt = new DLList<T>::box{data, fisrt, nullptr};
        if(fisrt->next != nullptr)
        {
    ...
    {
        fisrt = last = new DLList<T>::box{data, nullptr, nullptr};
    }
    return *this;
}
...
template <class T>
typename DLList<T>::box* DLList<T>::getFisrt() const
{
    return fisrt;
}
